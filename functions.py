...
def get_data(blocks):
    ...
    return (n.asarray(machs), n.asarray(alts), n.asarray(sslips), n.asarray(mcenter))
...
def replaceBadChars(listofStrings):
    ...
    for name in listofStrings:
        list.append(
            pattern.sub("_", name[0:len(name) - 1]) + pattern.sub("", name[len(name) - 1])
        )
...
""" This function loop over the blocks and returns all the data in a list
---- OUTPUT
